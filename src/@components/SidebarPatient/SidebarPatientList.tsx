  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-5 pt-5 pb-4">
        <div className="font-extrabold text-brand-deep select-none text-2xl">{t('sidebar.patients')}</div>
        <button
          aria-label={showSearch ? t('aria.closeSearch') : t('aria.openSearch')}
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <input
              ...
              aria-label={t('aria.searchPatients')}
            />
          </div>
        </div>
      )}
      ...
      <ul className="flex-1 overflow-y-auto thin-scroll pr-1 space-y-1 pt-4">
        {loading && (
          <li className="px-4 py-2 text-xs text-gray-500">{t('sidebar.loading')}</li>
        )}
        {!loading &&
          filtered.map((p) => (
            <SidebarPatientItem
